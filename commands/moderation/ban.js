...
exports.run = async(client,message,args) => {
    ...
    if (!message.guild.me.permissions.has("BAN_MEMBERS")) return message.reply('Kullanıcıyı Banlayamıyorum Çünkü `BAN_MEMBERS` Yetkim Yok.');
    if (!message.member.hasPermission("BAN_MEMBERS")) return message.reply(":no_entry: Bu komutu kullanabilmek için `BAN_MEMBERS` yetkisine sahip olmanız gerek.");
    ...
    if(!kisi) return message.reply("Banlayacağım Kişiyi Belirtmen Gerekiyor `ID / @mention / username`")
